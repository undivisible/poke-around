...
#[test]
fn oauth_protected_resource_should_not_be_collapsed_to_health_check() {
    let state = AppState::new(PermissionMode::Full, false).expect("state should initialize");
    let port = start_server(state).expect("server should start");
    let mut stream = TcpStream::connect(("127.0.0.1", port)).expect("server should accept");
    stream
        .write_all(
            b"GET /.well-known/oauth-protected-resource/mcp HTTP/1.1\r\nHost: 127.0.0.1\r\n\r\n",
        )
        .expect("request should write");
    let mut response = String::new();
    stream
    ...
    assert!(response.starts_with("HTTP/1.1 200 OK"));
    assert!(response.contains(r#""authorization_servers":["https://poke.com"]"#));
    assert!(response.contains(r#""resource":"http://127.0.0.1/mcp""#));
    assert!(!response.contains(r#""ok":true"#));
}
